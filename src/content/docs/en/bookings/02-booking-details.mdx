...
Only valid transitions are shown (e.g. from **Assigned** you may see Pending Payment, Accepted, Completed, Cancelled; **Set status to Picked up** appears when the booking is Assigned and ready for the driver to mark as collected).

:::note
Only transitions allowed for the current status are shown in the Select Action dropdown.
:::
